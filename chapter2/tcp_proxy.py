...
def proxy_handler(client_socket, remote_host, remote_port, receive_first):
    ...
    # Loop for read from local, send to remote and send to local
    while True:
...
def hexdump(src, length=16):
    ...
    print b'\n'.join(result)
def receive_from(connect):
    buffer = ""
    
    # We set a 2 second timeout; depending on target this may need to be adjusted.
...
def request_handler(buffer):
    # include modifications to requests
    return buffer
...
def main():
    if len(sys.argv[1:]) != 5:
        print "Usage: ./proxy.py [localhost] [localport] [localhost] [remoteport] [receive_first]"
