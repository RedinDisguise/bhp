...
def run_command(command):
    ...
    #run the comman and get the output back
    try:
...
def client_handler(client_socket):
    ...
    #now we go into a loop if a command shell was requested
    if command:
        while True:
...
def main():
    ...
    if not listen and len(target) and port > 0:
        ...
        #this will block, so send CNTRL-D if not sending input
        #to stdin
    ...
    #depending on our command line options
    if listen:
        server_loop()
